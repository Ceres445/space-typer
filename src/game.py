...
class Game(arcade.Window):
    ...
    def create_word(self):
        ...
        occupied_rows = set()
        while True:
            row: int = random.randrange(self.word_rows_count)
            for word in self.word_list:
    ...
    def update(self, delta_time):
        ...
        if self.state == GameStates.RUNNING:
            ...
            if self.lives <= 0:
                ...
                if score_file.get("high_score") is None:
                    new_high_score: int = self.score
                else:
    ...
    def _get_leftmost_word_starting_with(self, character):
        ...
        if len(words_starting_with_given_character) == 0:
            ...
            leftmost_word = min(words_starting_with_given_character, key=lambda _word: _word.x)
    ...
    def on_key_press(self, key, modifiers):
        ...
        if self.focus_word is None:
            ...
            if self.focus_word.word[0].lower() == chr(key):
                self.focus_word.attack()
        if self.focus_word is not None and self.focus_word.word == "":
